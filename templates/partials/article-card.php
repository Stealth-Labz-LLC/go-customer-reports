...
<div class="card h-100 article-card">
    ...
    <div class="card-body d-flex flex-column">
        ...
        <p class="card-text text-muted flex-grow-1"><?= htmlspecialchars(mb_substr($article->excerpt, 0, 120)) ?>...</p>
        <?php endif; ?>
        <div class="d-flex justify-content-between align-items-center mt-auto pt-2">
            <?php if ($article->published_at): ?>
            <small class="text-muted"><i class="far fa-calendar-alt me-1"></i> <?= date('M j, Y', strtotime($article->published_at)) ?></small>
            <?php endif; ?>
            <a href="<?= $articleUrl ?>" class="btn btn-outline-success">Read More <i class="fas fa-arrow-right ms-1"></i></a>
        </div>
    </div>
</div>
